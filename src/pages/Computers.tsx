        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <div className="flex-1 md:flex-initial md:w-80">
              <SearchBar onSearch={setSearchQuery} placeholder="Pesquisar computadores..." />
            </div>
            <Button onClick={() => navigate('/carrinho')} className="bg-[#1e90ff] hover:bg-[#0a65c0] relative">
              <ShoppingCart className="w-5 h-5 mr-2" />
              Carrinho
              {totalItems > 0 && <span className="absolute -top-2 -right-2 bg-destructive text-white rounded-full w-6 h-6 flex items-center justify-center text-xs">{totalItems}</span>}
            </Button>
          </div>
        </div>
